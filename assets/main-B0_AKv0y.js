...
`)}var W=class{constructor(s){d(this,"options");d(this,"rules");d(this,"lexer");this.options=s||I}space(s){let e=this.rules.block.newline.exec(s);if(e&&e[0].length>0)return{type:"space",raw:e[0]}}code(s){let e=this.rules.block.code.exec(s);if(e){let t=e[0].replace(this.rules.other.codeRemoveIndent,"");return{type:"code",raw:e[0],codeBlockStyle:"indented",text:this.options.pedantic?t:M(t,`
...
${l}`:l,r=r?`${r}
${p}`:p;let g=this.lexer.state.top;if(this.lexer.state.top=!0,this.lexer.blockTokens(p,o,!0),this.lexer.state.top=g,t.length===0)break;let f=o.at(-1);if((f==null?void 0:f.type)==="code")break;if((f==null?void 0:f.type)==="blockquote"){let h=f,k=h.raw+`
`+t.join(`
`),u=this.blockquote(k);o[o.length-1]=u,n=n.substring(0,n.length-h.raw.length)+u.raw,r=r.substring(0,r.length-h.text.length)+u.text;break}else if((f==null?void 0:f.type)==="list"){let h=f,k=h.raw+`
`+t.join(`
`),u=this.list(k);o[o.length-1]=u,n=n.substring(0,n.length-f.raw.length)+u.raw,r=r.substring(0,r.length-h.raw.length)+u.raw,t=k.substring(o.at(-1).raw.length).split(`
`);continue}}return{type:"blockquote",raw:n,tokens:o,text:r}}}list(s){var t,n;let e=this.rules.block.list.exec(s);if(e){let r=e[1].trim(),o=r.length>1,a={type:"list",raw:"",ordered:o,start:o?+r.slice(0,-1):"",loose:!1,items:[]};r=o?`\\d{1,9}\\${r.slice(-1)}`:`\\${r}`,this.options.pedantic&&(r=o?r:"[*+-]");let i=this.rules.other.listItemRegex(r),c=!1;for(;s;){let p=!1,g="",f="";if(!(e=i.exec(s))||this.rules.block.hr.test(s))break;g=e[0],s=s.substring(g.length);let h=e[2].split(`
`,1)[0].replace(this.rules.other.listReplaceTabs,y=>" ".repeat(3*y.length)),k=s.split(`
`,1)[0],u=!h.trim(),b=0;if(this.options.pedantic?(b=2,f=h.trimStart()):u?b=e[1].length+1:(b=e[2].search(this.rules.other.nonSpaceChar),b=b>4?1:b,f=h.slice(b),b+=e[1].length),u&&this.rules.other.blankLine.test(k)&&(g+=k+`
...
`,1)[0],z;if(k=X,this.options.pedantic?(k=k.replace(this.rules.other.listReplaceNesting,"  "),z=k):z=k.replace(this.rules.other.tabCharGlobal,"    "),L.test(k)||U.test(k)||Ze.test(k)||y.test(k)||C.test(k))break;if(z.search(this.rules.other.nonSpaceChar)>=b||!k.trim())f+=`
`+z.slice(b);else{if(u||h.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||L.test(h)||U.test(h)||C.test(h))break;f+=`
`+k}!u&&!k.trim()&&(u=!0),g+=X+`
`,s=s.substring(X.length+1),h=z.slice(b)}}a.loose||(c?a.loose=!0:this.rules.other.doubleBlankLine.test(g)&&(c=!0)),a.items.push({type:"list_item",raw:g,task:!!this.options.gfm&&this.rules.other.listIsTask.test(f),loose:!1,text:f,tokens:[]}),a.raw+=g}let l=a.items.at(-1);if(l)l.raw=l.raw.trimEnd(),l.text=l.text.trimEnd();else return;a.raw=a.raw.trimEnd();for(let p of a.items){if(this.lexer.state.top=!1,p.tokens=this.lexer.blockTokens(p.text,[]),p.task){if(p.text=p.text.replace(this.rules.other.listReplaceTask,""),((t=p.tokens[0])==null?void 0:t.type)==="text"||((n=p.tokens[0])==null?void 0:n.type)==="paragraph"){p.tokens[0].raw=p.tokens[0].raw.replace(this.rules.other.listReplaceTask,""),p.tokens[0].text=p.tokens[0].text.replace(this.rules.other.listReplaceTask,"");for(let f=this.lexer.inlineQueue.length-1;f>=0;f--)if(this.rules.other.listIsTask.test(this.lexer.inlineQueue[f].src)){this.lexer.inlineQueue[f].src=this.lexer.inlineQueue[f].src.replace(this.rules.other.listReplaceTask,"");break}}let g=this.rules.other.listTaskCheckbox.exec(p.raw);if(g){let f={type:"checkbox",raw:g[0]+" ",checked:g[0]!=="[ ]"};p.checked=f.checked,a.loose?p.tokens[0]&&["paragraph","text"].includes(p.tokens[0].type)&&"tokens"in p.tokens[0]&&p.tokens[0].tokens?(p.tokens[0].raw=f.raw+p.tokens[0].raw,p.tokens[0].text=f.raw+p.tokens[0].text,p.tokens[0].tokens.unshift(f)):p.tokens.unshift({type:"paragraph",raw:f.raw,text:f.raw,tokens:[f]}):p.tokens.unshift(f)}}if(!a.loose){let g=p.tokens.filter(h=>h.type==="space"),f=g.length>0&&g.some(h=>this.rules.other.anyLine.test(h.raw));a.loose=f}}if(a.loose)for(let p of a.items){p.loose=!0;for(let g of p.tokens)g.type==="text"&&(g.type="paragraph")}return a}}html(s){let e=this.rules.block.html.exec(s);if(e)return{type:"html",block:!0,raw:e[0],pre:e[1]==="pre"||e[1]==="script"||e[1]==="style",text:e[0]}}def(s){let e=this.rules.block.def.exec(s);if(e){let t=e[1].toLowerCase().replace(this.rules.other.multipleSpaceGlobal," "),n=e[2]?e[2].replace(this.rules.other.hrefBrackets,"$1").replace(this.rules.inline.anyPunctuation,"$1"):"",r=e[3]?e[3].substring(1,e[3].length-1).replace(this.rules.inline.anyPunctuation,"$1"):e[3];return{type:"def",tag:t,raw:e[0],href:n,title:r}}}table(s){var a;let e=this.rules.block.table.exec(s);if(!e||!this.rules.other.tableDelimiter.test(e[2]))return;let t=_e(e[1]),n=e[2].replace(this.rules.other.tableAlignChars,"").split("|"),r=(a=e[3])!=null&&a.trim()?e[3].replace(this.rules.other.tableRowBlankLine,"").split(`
...
`)?"":`
`)+i.raw,l.text+=`
`+i.text,this.inlineQueue.pop(),this.inlineQueue.at(-1).src=l.text):t.push(i);continue}if(e){let l="Infinite loop on byte: "+e.charCodeAt(0);if(this.options.silent){console.error(l);break}else throw new Error(l)}}return this.state.top=!0,t}inline(e,t=[]){return this.inlineQueue.push({src:e,tokens:t}),t}inlineTokens(e,t=[]){var c,l,p,g,f;let n=e,r=null;if(this.tokens.links){let h=Object.keys(this.tokens.links);if(h.length>0)for(;(r=this.tokenizer.rules.inline.reflinkSearch.exec(n))!=null;)h.includes(r[0].slice(r[0].lastIndexOf("[")+1,-1))&&(n=n.slice(0,r.index)+"["+"a".repeat(r[0].length-2)+"]"+n.slice(this.tokenizer.rules.inline.reflinkSearch.lastIndex))}for(;(r=this.tokenizer.rules.inline.anyPunctuation.exec(n))!=null;)n=n.slice(0,r.index)+"++"+n.slice(this.tokenizer.rules.inline.anyPunctuation.lastIndex);let o;for(;(r=this.tokenizer.rules.inline.blockSkip.exec(n))!=null;)o=r[2]?r[2].length:0,n=n.slice(0,r.index+o)+"["+"a".repeat(r[0].length-o-2)+"]"+n.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);n=((l=(c=this.options.hooks)==null?void 0:c.emStrongMask)==null?void 0:l.call({lexer:this},n))??n;let a=!1,i="";for(;e;){a||(i=""),a=!1;let h;if((g=(p=this.options.extensions)==null?void 0:p.inline)!=null&&g.some(u=>(h=u.call({lexer:this},e,t))?(e=e.substring(h.raw.length),t.push(h),!0):!1))continue;if(h=this.tokenizer.escape(e)){e=e.substring(h.raw.length),t.push(h);continue}if(h=this.tokenizer.tag(e)){e=e.substring(h.raw.length),t.push(h);continue}if(h=this.tokenizer.link(e)){e=e.substring(h.raw.length),t.push(h);continue}if(h=this.tokenizer.reflink(e,this.tokens.links)){e=e.substring(h.raw.length);let u=t.at(-1);h.type==="text"&&(u==null?void 0:u.type)==="text"?(u.raw+=h.raw,u.text+=h.text):t.push(h);continue}if(h=this.tokenizer.emStrong(e,n,i)){e=e.substring(h.raw.length),t.push(h);continue}if(h=this.tokenizer.codespan(e)){e=e.substring(h.raw.length),t.push(h);continue}if(h=this.tokenizer.br(e)){e=e.substring(h.raw.length),t.push(h);continue}if(h=this.tokenizer.del(e)){e=e.substring(h.raw.length),t.push(h);continue}if(h=this.tokenizer.autolink(e)){e=e.substring(h.raw.length),t.push(h);continue}if(!this.state.inLink&&(h=this.tokenizer.url(e))){e=e.substring(h.raw.length),t.push(h);continue}let k=e;if((f=this.options.extensions)!=null&&f.startInline){let u=1/0,b=e.slice(1),y;this.options.extensions.startInline.forEach(C=>{y=C.call({lexer:this},b),typeof y=="number"&&y>=0&&(u=Math.min(u,y))}),u<1/0&&u>=0&&(k=e.substring(0,u+1))}if(h=this.tokenizer.inlineText(k)){e=e.substring(h.raw.length),h.raw.slice(-1)!=="_"&&(i=h.raw.slice(-1)),a=!0;let u=t.at(-1);(u==null?void 0:u.type)==="text"?(u.raw+=h.raw,u.text+=h.text):t.push(h);continue}if(e){let u="Infinite loop on byte: "+e.charCodeAt(0);if(this.options.silent){console.error(u);break}else throw new Error(u)}}return t}},G=class{constructor(s){d(this,"options");d(this,"parser");this.options=s||I}space(s){return""}code({text:s,lang:e,escaped:t}){var o;let n=(o=(e||"").match(_.notSpaceStart))==null?void 0:o[0],r=s.replace(_.endingNewline,"")+`
...
<body>
  <h1>${x(e)}</h1>
`;const o=new Map;for(const i of s)for(const c of i.contentBlocks||[])if(c.type==="tool_result"){const l=c;o.set(l.toolUseId,l)}const a=new Set;for(const i of s){const c=(i.role||"unknown").toLowerCase(),l=i.timestamp?new Date(i.timestamp).toLocaleString():"",p=c==="user",g=c==="assistant",f=c==="system";if(p&&!n.user||g&&!n.assistant||f&&!n.system||!p&&!g&&!f)continue;let h="",k=!1;if(i.contentBlocks&&i.contentBlocks.length>0)for(const u of i.contentBlocks)switch(u.type){case"text":{u.text&&u.text.trim()&&(h+=`      <div class="text">${te(u.text)}</div>
`,k=!0);break}case"thinking":{n.thinking&&(h+=Qt(u),k=!0);break}case"tool_use":{if(n.toolUse){const b=u,y=o.get(b.toolUseId);y&&a.add(b.toolUseId),h+=Vt(b,y,n),k=!0}break}case"tool_result":{const b=u;n.toolResult&&!a.has(b.toolUseId)&&(h+=Xt(b),k=!0);break}}(!i.contentBlocks||i.contentBlocks.length===0)&&i.text&&i.text.trim()&&(h+=`      <div class="text">${te(i.text)}</div>
...
`)}return r+=`  <div class="meta">Exported from THINKT on ${new Date().toLocaleString()}</div>
        ...
        <div class="thinking-header">
          ...
          <span class="thinking-label">Thinking${s.durationMs?` (${j(s.durationMs)})`:""}</span>
        </div>
        <div class="thinking-content">${x(s.thinking||"")}</div>
      </div>
`}function Vt(s,e,t){const n=JSON.stringify(s.toolInput,null,2),r=ue(s.toolName,s.toolInput),o=r?`<span class="tool-summary">(${x(r)})</span>`:"";let a="";if(e&&t.toolResult){const i=e.isError?"tool-result--error":"",c=e.isError?"Error":"Result",l=e.durationMs?` (${j(e.durationMs)})`:"";a=`
...
`}function Xt(s){const e=s.isError?"tool tool-error":"tool tool-result",t=s.isError?"Error":"Result",n=s.durationMs?` (${j(s.durationMs)})`:"";return`      <div class="${e}">
        <div class="tool-header">
          <span class="tool-toggle">▶</span>
...
`);break}case"thinking":{if(n.thinking){const u=k,b=u.durationMs?` (${j(u.durationMs)})`:"";f+=`<details>
...
`),!f.trim())continue;const h=i.timestamp?new Date(i.timestamp).toLocaleString():"";r+=`---

`,r+=`## ${c.charAt(0).toUpperCase()+c.slice(1)}`,h&&(r+=` (${h})`),r+=`

`,r+=f}return r+=`---

*Exported from THINKT on ${new Date().toLocaleString()}*
`,r}class Jt{constructor(e){d(this,"container");d(this,"contentContainer");d(this,"filterContainer");d(this,"toolbarContainer");d(this,"stylesInjected",!1);d(this,"client",null);d(this,"filterState",{user:!0,assistant:!0,thinking:!1,toolUse:!1,toolResult:!1,system:!1});d(this,"currentProjectPath",null);d(this,"currentEntryCount",0);d(this,"currentEntries",[]);d(this,"toolResultIndex",new Map);d(this,"inlinedToolResults",new Set);d(this,"boundFilterHandlers",new Map);d(this,"exportDropdownOpen",!1);this.container=e.elements.container,this.client=e.client??null,this.init()}init(){this.injectStyles(),this.container.className="thinkt-conversation-view",this.createStructure(),this.setupFilters()}injectStyles(){if(this.stylesInjected)return;const e="thinkt-conversation-view-styles";if(!document.getElementById(e)){const t=document.createElement("style");t.id=e,t.textContent=at,document.head.appendChild(t)}this.stylesInjected=!0}createStructure(){this.toolbarContainer=document.createElement("div"),this.toolbarContainer.className="thinkt-conversation-view__toolbar",this.renderToolbar(),this.container.appendChild(this.toolbarContainer),this.filterContainer=document.createElement("div"),this.filterContainer.className="thinkt-conversation-view__filters",this.renderFilterBar(),this.container.appendChild(this.filterContainer),this.contentContainer=document.createElement("div"),this.contentContainer.className="thinkt-conversation-view__content",this.container.appendChild(this.contentContainer),this.contentContainer.addEventListener("click",e=>this.handleContentClick(e)),this.showEmpty()}handleContentClick(e){const t=e.target,n=t.closest(".thinkt-thinking-block__header");if(n){const a=n.closest(".thinkt-thinking-block");a==null||a.classList.toggle("expanded");return}const r=t.closest(".thinkt-tool-call__summary");if(r){const a=r.closest(".thinkt-tool-call");a==null||a.classList.toggle("expanded");return}const o=t.closest(".thinkt-copy-btn");if(o){this.handleCopy(o);return}}async handleCopy(e){let t="";const n=e.dataset.copyAction;if(n==="code"){const r=e.closest(".thinkt-code-block"),o=r==null?void 0:r.querySelector("code");t=(o==null?void 0:o.textContent)??""}else if(n==="text"){const r=e.closest(".thinkt-conversation-entry__text");if(r){const o=r.cloneNode(!0);o.querySelectorAll(".thinkt-copy-btn").forEach(a=>a.remove()),t=o.textContent??""}}else if(n==="detail"){const r=e.closest(".thinkt-tool-call__detail-content");if(r){const o=r.cloneNode(!0);o.querySelectorAll(".thinkt-copy-btn").forEach(a=>a.remove()),t=o.textContent??""}}if(t)try{await navigator.clipboard.writeText(t);const r=e.textContent;e.textContent="✓",setTimeout(()=>{e.textContent=r},1500)}catch{}}renderToolbar(){const e=this.currentProjectPath??"No project selected",t=this.currentEntryCount;this.toolbarContainer.innerHTML=`
    ...
    `,this.setupToolbarActions()}setupToolbarActions(){const e=this.toolbarContainer.querySelector("#toolbar-open-btn"),t=this.toolbarContainer.querySelector("#toolbar-dropdown");!e||!t||(e.addEventListener("click",n=>{n.stopPropagation(),t.classList.toggle("open")}),document.addEventListener("click",()=>{t.classList.remove("open")}),t.querySelectorAll(".thinkt-conversation-view__toolbar-dropdown-item").forEach(n=>{n.addEventListener("click",r=>{r.stopPropagation();const o=n.dataset.action;this.handleToolbarAction(o??""),t.classList.remove("open")})}))}async handleToolbarAction(e){if(!this.currentProjectPath)return;const t={finder:"finder",ghostty:"ghostty",vscode:"vscode",cursor:"cursor",terminal:"terminal"};if(e==="copy"){try{await navigator.clipboard.writeText(this.currentProjectPath)}catch{}return}const n=t[e];if(n)if(this.client)try{await this.client.openIn(n,this.currentProjectPath)}catch{try{await navigator.clipboard.writeText(this.currentProjectPath)}catch{}}else try{await navigator.clipboard.writeText(this.currentProjectPath)}catch{}}setProjectPath(e,t){this.currentProjectPath=e,t!==void 0&&(this.currentEntryCount=t),this.renderToolbar()}renderFilterBar(){const e=this.currentEntries.length>0;this.filterContainer.innerHTML=`
    ...
    `}renderThinkingBlock(e,t){const n=j(t),r=n?`<span class="thinkt-thinking-block__duration">(${x(n)})</span>`:"",o=e.replace(/\n/g," ").slice(0,80),a=o?`<span class="thinkt-thinking-block__preview">${x(o)}${e.length>80?"…":""}</span>`:"";return`
    ...
    `}renderToolCall(e,t,n){const r=ue(t,n),o=r?`<span class="thinkt-tool-call__arg">(${x(r)})</span>`:"",a=this.toolResultIndex.get(e);let i,c="",l="";if(a){this.inlinedToolResults.add(e),a.isError?i='<span class="thinkt-tool-call__status thinkt-tool-call__status--error">✗</span>':i='<span class="thinkt-tool-call__status thinkt-tool-call__status--ok">✓</span>';const g=j(a.durationMs);g&&(l=`<span class="thinkt-tool-call__duration">${x(g)}</span>`);const f=a.isError?" thinkt-tool-call__detail-result--error":"",h=a.isError?"Error":"Result";c=`
        <div class="thinkt-tool-call__detail-section thinkt-tool-call__detail-result${f}">
          <div class="thinkt-tool-call__detail-label">${h}</div>
